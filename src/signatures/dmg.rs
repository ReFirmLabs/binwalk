...
pub fn dmg_parser(file_data: &Vec<u8>, offset: usize) -> Result<signatures::common::SignatureResult, signatures::common::SignatureError> {
    ...
    if let Ok(dmg_footer) = parse_dmg_footer(&file_data[offset..]) {
        ...
        if dmg_footer.data_offset == 0 {
            ...
            if start_xml_signature < file_data.len() && end_xml_signature < file_data.len() {
                ...
                if let Ok(xml_signature) = String::from_utf8(file_data[start_xml_signature..end_xml_signature].to_vec()) {
