...
use crate::structures::zip::{ parse_zip_header, parse_eocd_header };
...
pub fn zip_parser(file_data: &Vec<u8>, offset: usize) -> Result<signatures::common::SignatureResult, signatures::common::SignatureError> {
    ...
    if let Ok(_) = parse_zip_header(&file_data[offset..]) {
        ...
        if let Ok(zip_info) = find_zip_eof(&file_data, offset) {
            ...
            result.description = format!("{}, file count: {}, total size: {} bytes", result.description, zip_info.file_count, result.size);
            return Ok(result);
        }
    }
 ...
 */
fn find_zip_eof(file_data: &Vec<u8>, offset: usize) -> Result<ZipEOCDInfo, signatures::common::SignatureError> {
