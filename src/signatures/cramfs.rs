...
pub fn cramfs_parser(file_data: &Vec<u8>, offset: usize) -> Result<signatures::common::SignatureResult, signatures::common::SignatureError> {
    ...
    if offset >= SIGNATURE_OFFSET {
        ...
        if let Ok(cramfs_header) = parse_cramfs_header(&file_data[result.offset..]) {
            ...
            if file_data.len() >= (result.offset + result.size) {
                 ...
                 */
                let mut cramfs_image: Vec<u8> = file_data[result.offset..result.offset+result.size].to_vec().clone();
                ...
                result.description = format!("{}, {} endian, {} files, total size: {} bytes{}", result.description,
