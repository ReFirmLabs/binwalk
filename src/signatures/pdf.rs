...
pub fn pdf_parser(file_data: &Vec<u8>, offset: usize) -> Result<signatures::common::SignatureResult, signatures::common::SignatureError> {
        ...
        if version_minor <= ASCII_NINE && version_minor >= ASCII_ZERO {
            if percent == ASCII_PERCENT {
                if newline == ASCII_NEWLINE || newline == ASCII_CARRIGE_RETURN {
                    ...
                    result.description = format!("{}, version 1.{}, created on a {} system", result.description, version_minor-ASCII_ZERO, os_string);
