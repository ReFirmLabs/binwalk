...
use crate::structures::lz4::{ parse_lz4_file_header, parse_lz4_block_header };
...
pub fn lz4_parser(file_data: &Vec<u8>, offset: usize) -> Result<signatures::common::SignatureResult, signatures::common::SignatureError> {
    ...
    if let Ok(lz4_file_header) = parse_lz4_file_header(&file_data[offset..]) {
        ...
        if lz4_file_header.header_size < available_data {
            ...
            if let Ok(lz4_data_size) = get_lz4_data_size(&file_data[lz4_file_header.header_size..], lz4_file_header.block_checksum_present) {
...
fn get_lz4_data_size(lz4_data: &[u8], checksum_present: bool) -> Result<usize, signatures::common::SignatureError> {
    ...
    while lz4_data.len() > lz4_data_size {
        ...
        match parse_lz4_block_header(&lz4_data[lz4_data_size..], checksum_present) {
            ...
            Ok(block_header) => {
                lz4_data_size += block_header.header_size + block_header.data_size + block_header.checksum_size;
