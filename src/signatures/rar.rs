...
pub fn rar_parser(file_data: &Vec<u8>, offset: usize) -> Result<signatures::common::SignatureResult, signatures::common::SignatureError> {
    ...
    if let Ok(rar_header) = parse_rar_archive_header(&file_data[offset..]) {
        ...
        result.description = format!("{}, version: {}, total size: {} bytes{}", result.description, rar_header.version, result.size, extra_description);
        return Ok(result);
    }
...
fn get_rar_size(file_data: &[u8], rar_version: usize) -> Result<usize, signatures::common::SignatureError> {
