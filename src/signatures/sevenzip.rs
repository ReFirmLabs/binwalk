...
pub fn sevenzip_parser(file_data: &Vec<u8>, offset: usize) -> Result<signatures::common::SignatureResult, signatures::common::SignatureError> {
    ...
    if let Ok(sevenzip_header) = parse_7z_header(&file_data[offset..]) {
        ...
        let next_crc_start: usize = offset + sevenzip_header.header_size + sevenzip_header.next_header_offset;
        ...
        if next_crc_end <= file_data.len() {
            ...
            let calculated_next_crc: usize = crc32(&file_data[next_crc_start..next_crc_end]) as usize;
            ...
            if calculated_next_crc == sevenzip_header.next_header_crc {
                ...
                let total_size: usize = sevenzip_header.header_size + sevenzip_header.next_header_offset + sevenzip_header.next_header_size;
                ...
                return Ok(signatures::common::SignatureResult {
                                                        ...
                                                        description: format!("{}, version {}.{}, total size: {} bytes", DESCRIPTION,
