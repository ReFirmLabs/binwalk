...
pub fn parse_zstd_header(zstd_data: &[u8]) -> Result<ZSTDHeader, structures::common::StructureError> {
    ...
    let zstd_header_structure = vec![
        ("magic", "u32"),
        ("frame_header_descriptor", "u8"),
    ];

    let mut zstd_info = ZSTDHeader { fixed_header_size: ZSTD_HEADER_MIN_SIZE, ..Default::default() };

    // Sanity check the size of available data
    if zstd_data.len() >= ZSTD_HEADER_MIN_SIZE {

        // Parse the ZSTD header
        let zstd_header = structures::common::parse(&zstd_data[0..ZSTD_HEADER_MIN_SIZE], &zstd_header_structure, "little");
        ...
        if (zstd_header["frame_header_descriptor"] & FRAME_UNUSED_BITS_MASK) == 0 {
            ...
            zstd_info.content_checksum_present = (zstd_header["frame_header_descriptor"] & CONTENT_CHECKSUM_MASK) != 0;
            
            // If this flag is set, then the window descriptor byte is not present
            zstd_info.single_segment_flag = (zstd_header["frame_header_descriptor"] & SINGLE_SEGMENT_MASK) != 0;
            
            // Indicates if the frame content field is present, and if so, how big it is
            zstd_info.frame_content_flag = (zstd_header["frame_header_descriptor"] & FRAME_CONTENT_MASK) >> FRAME_CONTENT_SHIFT;
...
pub fn parse_block_header(block_data: &[u8]) -> Result<ZSTDBlockHeader, structures::common::StructureError> {
    ...
    let zstd_block_header_structure = vec![
        ("info_bits", "u24"),
    ];

    let mut block_info = ZSTDBlockHeader { header_size: ZSTD_BLOCK_HEADER_SIZE, ..Default::default() };

    if block_data.len() >= ZSTD_BLOCK_HEADER_SIZE {

        // Parse the block header
        let block_header = structures::common::parse(block_data, &zstd_block_header_structure, "little");
        ...
        block_info.block_type = (block_header["info_bits"] & ZSTD_BLOCK_TYPE_MASK) >> ZSTD_BLOCK_TYPE_SHIFT;
        block_info.block_size = (block_header["info_bits"] & ZSTD_BLOCK_SIZE_MASK) >> ZSTD_BLOCK_SIZE_SHIFT;
