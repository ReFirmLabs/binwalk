...
pub fn parse_lzma_header(lzma_data: &[u8]) -> Result<LZMAHeader, structures::common::StructureError> {
    ...
    let mut lzma_hdr_info = LZMAHeader { ..Default::default() };
    ...
    if available_data > lzma_header_size {
        ...
        let lzma_header = structures::common::parse(&lzma_data[0..lzma_header_size], &lzma_structure, "little");
        ...
        if lzma_header["null_byte"] == 0 {
            if lzma_header["decompressed_size"] > MIN_SUPPORTED_DECOMPRESSED_SIZE {
                if lzma_header["decompressed_size"] == LZMA_STREAM_SIZE || lzma_header["decompressed_size"] < MAX_SUPPORTED_DECOMPRESSED_SIZE {
