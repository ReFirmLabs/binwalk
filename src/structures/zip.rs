...
pub fn parse_zip_header(zip_data: &[u8]) -> Result<bool, structures::common::StructureError> {
    ...
    if zip_data.len() > ZIP_LOCAL_FILE_HEADER_MIN_SIZE {
        ...
        let zip_local_file_header = structures::common::parse(
            &zip_data[zip_local_file_start..zip_local_file_end],
            &zip_local_file_structure,
            "little",
        );
...
pub fn parse_eocd_header(
    ...
    if eocd_data.len() >= eocd_end {
        ...
        let zip_eocd_header = structures::common::parse(
            &eocd_data[eocd_start..eocd_end],
            &zip_eocd_structure,
            "little",
        );
