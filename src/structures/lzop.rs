...
pub fn parse_lzop_file_header(lzop_data: &[u8]) -> Result<LZOPFileHeader, structures::common::StructureError> {
    ...
    let mut lzop_info = LZOPFileHeader { ..Default::default() };

    // Sanity check the size of available data
    if lzop_data.len() >= LZO_HEADER_MAX_SIZE {
        // Parse the first part of the header
        let lzo_header_p1 = structures::common::parse(&lzop_data[0..LZO_HEADER_SIZE_P1], &lzo_structure_p1, "big");

        // Sanity check the methods field
        if allowed_methods.contains(&lzo_header_p1["method"]) {

            // Sanity check the header version numbers
            if lzo_header_p1["version"] <= LZO_MAX_VERSION && lzo_header_p1["version"] >= lzo_header_p1["version_needed"] {
                ...
                let lzo_header_p2 = structures::common::parse(&lzop_data[header_p2_start..header_p2_end], &lzo_structure_p2, "big");

                // Calculate the total header size; compressed data blocks will immediately follow
                lzop_info.header_size = header_p2_end + lzo_header_p2["file_name_length"] +  LZO_CHECKSUM_SIZE;

                // Check if block headers include an optional compressed data checksum field
                lzop_info.block_checksum_present = (lzo_header_p1["flags"] & FLAG_ADLER32_C & FLAG_CRC32_C) != 0;
...
pub fn parse_lzop_block_header(lzo_data: &[u8], compressed_checksum_present: bool) -> Result<LZOPBlockHeader, structures::common::StructureError> {
    ...
    if lzo_data.len() >= BLOCK_HEADER_SIZE {
        ...
        let block_header = structures::common::parse(&lzo_data[0..BLOCK_HEADER_SIZE], &block_structure, "big");
        ...
        if block_header["compressed_size"] != 0 &&
           ...
           block_header["uncompressed_size"] <= MAX_UNCOMPRESSED_BLOCK_SIZE {
            ...
            let mut block_hdr_info = LZOPBlockHeader { ..Default::default() };
...
pub fn parse_lzop_eof_marker(eof_data: &[u8]) -> Result<usize, structures::common::StructureError> {
    ...
    if eof_data.len() >= EOF_MARKER_SIZE {
        ...
        let eof_marker = structures::common::parse(&eof_data[0..EOF_MARKER_SIZE], &eof_structure, "big");
