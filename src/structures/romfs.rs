...
pub fn parse_romfs_header(romfs_data: &[u8]) -> Result<RomFSHeader, structures::common::StructureError> {
    ...
    if romfs_data.len() >= header_size {
        ...
        let header = structures::common::parse(&romfs_data[0..header_size], &header_structure, "big");
...
pub fn parse_romfs_file_entry(romfs_data: &[u8]) -> Result<RomFSFileHeader, structures::common::StructureError> {
    ...
    if romfs_data.len() >= file_header_size {
        ...
        let file_entry_header = structures::common::parse(&romfs_data[0..file_header_size], &file_header_structure, "big");
        ...
        if file_name.len() > 0 {
            ...
            let mut file_header = RomFSFileHeader { ..Default::default() };
            ...
            file_header.next_header_offset = file_entry_header["next_header_offset"] & NEXT_OFFSET_MASK;
