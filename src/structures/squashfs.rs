...
pub fn parse_squashfs_header(sqsh_data: &[u8]) -> Result<SquashFSHeader, structures::common::StructureError> {
    ...
    let mut sqsh_header = SquashFSHeader { endianness: "little".to_string(), ..Default::default() };
    ...
    if sqsh_data.len() > MIN_SQUASHFS_HEADER_SIZE {
         ...
         */
        let mut squashfs_version: u16 = u16::from_le_bytes(sqsh_data[SQUASHFS_VERSION_START..SQUASHFS_VERSION_END].try_into().unwrap());
        ...
        if squashfs_version == 0 || squashfs_version > MAX_SQUASHFS_VERSION {
            ...
            squashfs_version = u16::from_be_bytes(sqsh_data[SQUASHFS_VERSION_START..SQUASHFS_VERSION_END].try_into().unwrap());
        }
        ...
        if squashfs_version <= MAX_SQUASHFS_VERSION && squashfs_version > 0 {
            ...
            if squashfs_version == 4 {
                ...
                squashfs_header = structures::common::parse(&sqsh_data, &squashfs_v4_structure, &sqsh_header.endianness);
            } else {
                squashfs_header_size = structures::common::size(&squashfs_v3_structure);
                squashfs_header = structures::common::parse(&sqsh_data, &squashfs_v3_structure, &sqsh_header.endianness);
            }
            ...
            if sqsh_header.image_size > MIN_SQUASHFS_HEADER_SIZE {
                ...
                if squashfs_header["block_size"] > 0 && squashfs_header["block_log"] == (squashfs_header["block_size"].ilog2() as usize) {
...
pub fn parse_squashfs_uid_entry(uid_data: &[u8], version: usize, endianness: &String) -> Result<usize, structures::common::StructureError> {
    const SQUASHFS_V3_UID_ENTRY_SIZE: usize = 4;
    const SQUASHFS_V4_UID_ENTRY_SIZE: usize = 8;
    
    let squashfs_v4_uid_table_structure = vec![
        ("uid_block_ptr", "u64"),
    ];
    
    let squashfs_v3_uid_table_structure = vec![
        ("uid_block_ptr", "u32"),
    ];
    ...
    if uid_data.len() >= uid_entry_size {
        ...
        if version == 4 {
            uid_table = structures::common::parse(&uid_data[0..uid_entry_size], &squashfs_v4_uid_table_structure, endianness);
        } else {
            uid_table = structures::common::parse(&uid_data[0..uid_entry_size], &squashfs_v3_uid_table_structure, endianness);
        }
