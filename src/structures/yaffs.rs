...
pub fn parse_yaffs_obj_header(header_data: &[u8], endianness: &str) -> Result<YAFFSObject, structures::common::StructureError> {
    ...
    if header_data.len() >= OBJ_STRUCT_SIZE {
        let obj_header = structures::common::parse(&header_data[0..OBJ_STRUCT_SIZE], &yaffs_object_structure, endianness);
        
        // Validate that the header looks sane
        if allowed_types.contains(&obj_header["type"]) && (obj_header["parent_id"] > 0) && (obj_header["name_checksum"] == UNUSED) {
...
pub fn parse_yaffs_file_header(header_data: &[u8], endianness: &str) -> Result<YAFFSFileHeader, structures::common::StructureError> {
    ...
    if header_data.len() >= INFO_STRUCT_SIZE {
        let file_info = structures::common::parse(&header_data[0..INFO_STRUCT_SIZE], &yaffs_file_info, endianness);
