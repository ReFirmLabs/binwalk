...
pub fn parse_lz4_file_header(lz4_data: &[u8]) -> Result<LZ4FileHeader, structures::common::StructureError> {
    ...
    let lz4_structure = vec![
        ("magic", "u32"),
        ("flags", "u8"),
        ("bd", "u8"),
    ];

    let mut lz4_hdr_info = LZ4FileHeader { ..Default::default() };

    // Sanity check the size of available data
    if lz4_data.len() >= LZ4_STRUCT_SIZE {
        // Parse the header
        let lz4_header = structures::common::parse(&lz4_data[0..LZ4_STRUCT_SIZE], &lz4_structure, "little");
        ...
        if (lz4_header["flags"] & FLAGS_RESERVED_MASK) == 0 && (lz4_header["bd"] & BD_RESERVED_MASK) == 0 {
            ...
            if crc_data_end < lz4_data.len() {
                ...
                let calculated_crc: u8 = ((xxhash_rust::xxh32::xxh32(&lz4_data[crc_data_start..crc_data_end], 0) >> 8) & 0xFF) as u8;
                ...
                if actual_crc == calculated_crc {
                    ...
                    lz4_hdr_info.block_checksum_present = (lz4_header["flags"] & FLAG_BLOCK_CHECKSUM_PRESENT) != 0;
                    lz4_hdr_info.content_checksum_present = (lz4_header["flags"] & FLAG_CONTENT_CHECKSUM_PRESENT) != 0;
...
pub fn parse_lz4_block_header(lz4_block_data: &[u8], checksum_present: bool) -> Result<LZ4BlockHeader, structures::common::StructureError> {
    ...
    let mut lz4_block = LZ4BlockHeader { ..Default::default() };

    // Sanity check on the available size of data
    if lz4_block_data.len() >= BLOCK_STRUCT_SIZE {
        // Parse the block header
        let block_header = structures::common::parse(&lz4_block_data[0..BLOCK_STRUCT_SIZE], &block_structure, "little");
