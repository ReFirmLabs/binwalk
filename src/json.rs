        ...
        Some(file_name) => {
            ...
            match serde_json::to_string_pretty(&results) {
                ...
                Ok(json) => {
                    ...
                    match fs::OpenOptions::new().create(true).read(true).write(true).open(&file_name) {
                        Err(e) => {
                            ...
                            match fp.seek(io::SeekFrom::End(0)) {
                                ...
                                Ok(pos) => {
                                    if pos == 0 {
                                        ...
                                        if let Err(e) = fp.seek(io::SeekFrom::Start(pos - (JSON_LIST_END.len() as u64))) {
                                            error!("Failed to seek to EOF-1 in JSON file: {}", e);
                                            return;
                                        } else {
                                            write_to_json_file(&fp, JSON_COMMA_SEPERATOR.to_string());
                                        }
                                    }
