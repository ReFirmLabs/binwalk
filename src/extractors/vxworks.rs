...
use crate::structures::vxworks::{ get_symtab_endianness, parse_symtab_entry, VxWorksSymbolTableEntry };
...
pub fn extract_symbol_table(file_data: &Vec<u8>, offset: usize, output_directory: Option<&String>) -> ExtractionResult {
    ...
    if symtab_entries.len() >= MIN_VALID_ENTRIES {
        ...
        if dry_run == false {
            ...
            match serde_json::to_string_pretty(&symtab_entries) {
                ...
                // Write JSON to file
                Ok(symtab_json) => {
                    result.success = create_file(&output_file_path, &symtab_json.clone().into_bytes(), 0, symtab_json.len());
                },
            }
