...
use crate::structures::bmp::{parse_bmp_file_header,get_dib_header_size};
...
pub fn extract_bmp_image(file_data: &[u8], offset: usize, output_directory: Option<&str>) -> ExtractionResult {
    ...
    if let Ok(bmp_file_header) = parse_bmp_file_header(&file_data[offset..]){
        ...
        if let Ok(bmp_header_size) = get_dib_header_size(&file_data[(offset+BMP_FILE_HEADER_SIZE)..]){
           ...
           if bmp_file_header.bitmap_bits_offset >= (BMP_FILE_HEADER_SIZE+bmp_header_size){
                ...
                if output_directory.is_some(){
                    ...
                    result.success = chroot.carve_file(OUTFILE_NAME, file_data, offset, bmp_file_header.size);
                }
           }
        }
    }
